...
{
	RegConsoleCmd("sm_n2e", Command_Noob2Easy, "Usage: sm_n2e <HP> - Calculate your secret code based on your HP amount");
}

public Action Command_Noob2Easy(int client, int args)
{
	// If client does not provide HP amount
	if (args == 0)
	{
	...
	// Store client HP amount as sBuffer
	char sBuffer[16];
	...
	if (strlen(sBuffer) > 0)
		StringToIntEx(sBuffer, input);
	
	// If converted user input returns 0 (empty), tell client to enter a number
	if (input == 0)
	{
